...
// The route only enqueues; a worker (embedded or standalone) claims the job
// and runs the pipeline. Safe to call from any API replica.
...
export async function analyzeRoutes(app: FastifyInstance): Promise<void> {
  app.post('/v1/projects/:id/analyze', async (req, reply) => {
    ...
    // Idempotent while active: re-attach instead of double-analyzing.
    const active = await jobs.findActiveJob('analyze', project.id);
    if (active) {
      await db.updateProject(project.id, { activeJob: { id: active.id, type: 'analyze' } });
      reply.code(202);
    ...
    const job = await jobs.createJob('analyze', `Analyze ${project.name}`, {
