...
// Local mode serves bytes from disk; R2 mode redirects media to presigned
// URLs so video bytes never proxy through the API tier.
...
async function projectOrNull(req: FastifyRequest, reply: FastifyReply): Promise<Project | null> {
...
export async function clipRoutes(app: FastifyInstance): Promise<void> {
  app.get('/v1/projects/:id/clips', async (req, reply) => {
    const project = await projectOrNull(req, reply);
    if (!project) return;
  ...
  app.get('/v1/projects/:id/source', async (req, reply) => {
    ...
    if (isR2() && project.media.r2Key) {
      const url = await getStorage().downloadUrl(project.media.r2Key);
  ...
  app.get('/v1/projects/:id/poster/:clipId', async (req, reply) => {
    ...
    if (isR2()) {
      const url = await getStorage().downloadUrl(keys.poster(project.id, clipId));
      if (url) return reply.redirect(url);
    }
